...
global.jsonconf = JSON.parse(fs.readFileSync(process.env.HOME+'/.ark.json', 'utf8'));
...
var nethash_cb = function(error, response, body) {
	...
	winston.log('info','Sending '+amount+' ('+dispamount+') to address: '+sendadr+' from '+max.address + ' (' + max.delegate+')');
        var transaction = ark.transaction.createTransaction(sendadr, amount, null, max.passphrase, max.secondpassphrase);
...
for (var i = 0; i < global.jsonconf.accounts.length; i++) {
	request('https://api.arknode.net/api/accounts/getBalance?address='+global.jsonconf.accounts[i].address, getbalances.bind(null, i));
}
